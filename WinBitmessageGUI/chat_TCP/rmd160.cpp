...
/********************************************************************/
void ripmd::MDcompress(word *MDbuf, word *X)
...
int ripmd::calc(LPBYTE out_hash, LPBYTE in_buff, DWORD64 in_size)
{
	DWORD64 ts = in_size;
	word* part = (word*)&ts;
	...
	MDfinish(tmp_hash, in_buff, part[0], part[1]);

	tmp_hash[0] = htonl(tmp_hash[0]);
	tmp_hash[1] = htonl(tmp_hash[1]);
	tmp_hash[2] = htonl(tmp_hash[2]);
	tmp_hash[3] = htonl(tmp_hash[3]);
	tmp_hash[4] = htonl(tmp_hash[4]);


	memcpy_s(out_hash, RIPMD160_SIZE, tmp_hash, RIPMD160_SIZE);
	
	ZeroMemory(tmp_hash, 32);
...
/********************************************************************/
void ripmd::MDfinish(word *MDbuf, byte *string, word lswlen, word mswlen)
