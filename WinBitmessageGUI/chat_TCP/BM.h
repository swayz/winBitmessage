...
namespace BM { //BitMessage (BM is easier to type :P)
	...
	DWORD obj_msg(PBM_ENC_PL_256 in, DWORD in_size);

//	DWORD obj_broadcast(); // <- v3 not suppored


	//DWORD obj_prop_server();
	//
	//VOID CALLBACK prop_obj_APCProc(_In_ ULONG_PTR dwParam);

	//DWORD propagate_obj(PBM_MSG_HDR msg, DWORD obj_type, DWORD pl_size, BOOL init);
	...
	void send_msg(DWORD from_id, DWORD to_id, LPWSTR subject, LPWSTR body);
