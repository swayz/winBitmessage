...
size_t BM::encodeVarint(uint64_t value, uint8_t* output) {
	...
	{
		output[0] = 0xFD;
		*((uint16_t *)&output[1]) = htons(value);
		ret_size = 3;
	}
	else if (value <= 0xFFFFFFFF)
	{
		output[0] = 0xFE;
		*((uint32_t *)&output[1]) = htonl(value);
		ret_size = 5;
	}
	else
	{
...
DWORD64 BM::do_pow_proc(PBM_POW_THREAD_DETAILS in)
{
	...
	*/
	...
	int64_t target = (int64_t)calc_pow_target(in->TTL, payloadLength, 1000, 1000);
...
DWORD BM::check_pow(LPBYTE payload, DWORD in_size, DWORD64 TTL)
{
	...
	Encryption::create_hash((LPSTR)initialHash, dataToCheck, payloadLength, NULL, NULL, CALG_SHA_512);
	...
	*((DWORD64*)tmp_buff) = BM::swap64(nonce);
	...
	uint64_t target = calc_pow_target(TTL, payloadLength, 1000, 1000);
	...
	if (Encryption::base58_decode(tmp_buff, &tmp_buff_size, b58, b58_size) && Encryption::base58_decode(tmp_buff, &tmp_buff_size, b58, b58_size))
	{
		...
		if (Utils::mem_cmp((LPBYTE)&hash_buff[64], 4, &tmp_buff[chcksum], 4))
		{
			...
			if (out_addr != FALSE)
			{
				...
				out_addr->version = version;
				out_addr->stream = stream;
...
DWORD BM::process_object(PBM_OBJECT object, DWORD object_size, LPBYTE vector)
{
	...
	version = BM::decodeVarint(object->objectVersion, 2, &v_offset);
	stream = BM::decodeVarint((uint8_t*)(((ULONG_PTR)object->objectVersion) + read_offset), 2, &s_offset);
	...
	LPBYTE pl = NULL;
	DWORD payload_size = object_size - (20 + read_offset);
	...
	case BM_OBJ_GETPUBKEY:
	{
		...
		//obj_hdr = (PBM_OBJECT)&t[sizeof(BM_MSG_HDR)]; //(PBM_OBJECT)&msg->payload;
		...
		if (found)
		{
			...
			//propagate_obj(msg_hdr, BM_OBJ_PUBKEY, BM_OPK_BS, TRUE);
			ret = TRUE;
	...
	case BM_OBJ_MSG:
	{
		...
		BM::obj_msg((PBM_ENC_PL_256)payload, payload_size);
...
//
//
//VOID CALLBACK BM::prop_obj_APCProc(_In_ ULONG_PTR dwParam)
//{
//	//DWORD max_send = 4;
//	PBM_MSG_HDR msg = (PBM_MSG_HDR)dwParam;
//	size_t size = htonl(*(uint32_t*)msg->length);
//
//	for (int i = 0; i < BM_MAX_CONNECTIONS; i++)
//	{
//		if (network::is_conn_ready(network::con_list->list[i]))
//		{
//
//			send(network::con_list->list[i]->s, (const char *)msg, size, NULL);
//			//DBGOUTa("Propogate -> object\n");
//			Sleep(200);
//			//max_send--;
//			//if (!max_send) break;
//		}
...
//	}
//	
//	ZEROFREE_((void*)msg, BM_RECV_BUFF_SIZE);
//
//
//}
//
//
//DWORD BM::propagate_obj(PBM_MSG_HDR msg, DWORD obj_type, DWORD pl_size, BOOL init)
//{
//
//	PBM_MSG_HDR new_msg_buff = (PBM_MSG_HDR)ALLOC_(BM_RECV_BUFF_SIZE);
//	memcpy_s(new_msg_buff, BM_RECV_BUFF_SIZE, msg, BM_RECV_BUFF_SIZE);
//
//	if (init)
//	{
//
//		//TODO: FiX Me
//
//		//DWORD obj_hdr_s = pl_size + sizeof(BM_MSG_HDR); // fix sizeof()
//		//DWORD obj_pl_size = pl_size - sizeof(BM_OBJECT);// fix sizeof()
//
//		//PBM_OBJECT obj_hdr = (PBM_OBJECT)&new_msg_buff->payload;
//
//		//LPBYTE obj_pl = (LPBYTE)((ULONG_PTR)(&new_msg_buff->payload) + sizeof(BM_OBJECT));
//
//		//BM::init_object(obj_hdr, obj_hdr_s, obj_type, obj_pl, obj_pl_size);
//
//		//BM::init_msg_hdr(new_msg_buff, pl_size, BM_MTS_OBJECT);
//	}
//
//	QueueUserAPC(BM::prop_obj_APCProc, BM::prop_thread_handle, (ULONG_PTR)new_msg_buff);
//	//prop_obj_APCProc((ULONG_PTR)msg);
//
//	return 0;
//}
//
...
DWORD BM::obj_msg(PBM_ENC_PL_256 in, DWORD in_size)
{

	if (BMDB::atmpt_msg_decrypt(in, in_size))
	{
...
DWORD BM::receive_addr_list(LPBYTE payload, DWORD in_size)
{
	...
	for (int i = 0; i < n_entrys && i < max_addrs; i++)
	{
		conn = NULL;
...
void BM::send_msg(DWORD from_id, DWORD to_id, LPWSTR subject, LPWSTR body)
{
	BM_MSG_ADDR addr = {};
	...
	if (!BMDB::address_find(to_id, NULL, &addr)) return;

	// save the message
	//BMDB::add_message(to_id, from_id, subject, body, 2, 0);
	...
	do {
		is_connected = network::has_connection();
		Sleep(1000);
	} while (!is_connected);
	...
	network::queue_obj(is_connected, vect_tag);
