...
BOOL BMDB::disc_all_conn_nodes(DWORD nid)
{
	...
	LPSTR st = "update nodes set is_connected = 0 where is_connected = 1";
...
BOOL BMDB::delete_row_from_table(DWORD row_id, LPSTR table)
{
	CHAR buf[512] = {};
	LPSTR st = "DELETE FROM %s WHERE id = %u";

	sqlite3_stmt * stmt = NULL;
	int rc = NULL;

	wsprintfA(buf, st, table, row_id);

	rc = sqlite3_prepare(BM::db, buf, -1, &stmt, NULL);
	...
	return (success == 101) ? TRUE : FALSE;

}



BOOL BMDB::set_is_node_connected(PBM_CONN conn, DWORD is_conn)
{
...
DWORD BMDB::add_message(DWORD to, DWORD from, LPSTR subject, LPSTR body, DWORD folder, DWORD inv_id)
{

	DWORD ret = FALSE;
	LPSTR st = "INSERT INTO msgs(id, inv_id, folder, to_id, from_id, subject, body, date) VALUES(NULL, ?, ?, ?, ?, ?, ?, ?)";
	sqlite3_stmt * stmt = NULL;
	int rc = NULL;
	rc = sqlite3_prepare(BM::db, st, -1, &stmt, NULL);
	...
	sqlite3_bind_text(stmt, 5, subject, -1, NULL);
	sqlite3_bind_text(stmt, 6, body, -1, NULL);
	...
	if (ret) ret = (BOOL)sqlite3_last_insert_rowid(BM::db);
...
DWORD BMDB::address_find(DWORD id, LPSTR addr, LPBYTE tag, PBM_MSG_ADDR pAddr)
{
		...
		do {
			...
			if (rc == SQLITE_ROW)
			{
				...
				else if (addr)
				{
					...
					if (Utils::mem_cmp((LPBYTE)addr, lstrlenA(addr), (LPBYTE)row_addr, row_addr_size))
						found = TRUE;
				...
				else if (tag)
				{
					...
					if (Utils::mem_cmp((LPBYTE)_addr->tag, 32, (LPBYTE)tag, 32))
						found = TRUE;
...
DWORD BMDB::address_add(PBM_MSG_ADDR in, LPWSTR label)
{
	...
	if (BMDB::address_find(0, in->readable, NULL, &a))
		return FALSE;
	...
	if (found) found = (DWORD)sqlite3_last_insert_rowid(BM::db);
...
DWORD BMDB::node_add(PBM_ADDR in)
{
	if (!in) return FALSE;
	...
	if (rc != SQLITE_OK)
	{
		...
		if (rc != SQLITE_OK)
		{
			...
			entry = (DWORD)sqlite3_last_insert_rowid(BM::db);// sqlite3_column_int(stmt, 0);
...
DWORD BMDB::vect_from_id(DWORD id, LPBYTE vect)
{
	...
	BOOL ret = FALSE;
	LPSTR st = "SELECT hash FROM INV WHERE ID = ?";
	...
	while (!ret && rc != SQLITE_ROW)
	{
		...
		_blob_size = sqlite3_column_bytes(stmt, 0);
		_blob = (LPBYTE)sqlite3_column_blob(stmt, 0);
		ret = TRUE;

		if (!_blob || !_blob_size)
			continue;

		Utils::copy_mem(vect, 32, _blob, _blob_size);
...
DWORD BMDB::vector_find(DWORD inv_id, LPBYTE vect, PBM_OBJECT obj, LPDWORD obj_size)
{

	if (!inv_id && !vect) return FALSE;

	DWORD ret = FALSE;
	//LPSTR st = "SELECT * FROM INVENTORY WHERE UID = ?";
	LPSTR st_uid = "SELECT ID, DATA FROM INV WHERE UID = ?";
	LPSTR st_id = "SELECT ID, DATA FROM INV WHERE ID = ?";
	LPSTR st = st_uid;
	...
	DWORD data_size = 0;
	LPBYTE blob = NULL;
	LPSTR uid = (LPSTR)LocalAlloc(LPTR, 512);
	//ZERO_(uid, 512);


	if (inv_id)
	{
		rc = sqlite3_bind_int(stmt, 1, inv_id);
	}
	else
	{
...
DWORD BMDB::vector_add(LPBYTE vect, PBM_OBJECT object, DWORD object_size)
{
	
	int v = BMDB::vector_find(NULL, vect, NULL, NULL);
